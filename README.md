...
A first prototype covering **Roosevelt Island, Manhattan** is in [`/prototype/roosevelt-island/`](prototype/roosevelt-island/).

Open `citiesonpalm.html` — one self-contained file, no server, no build step. 29 objects, 6 institutions, 2 offices, 32 sources. Read-only, no write path, zero third-party requests.

Nothing in it is invented. Every populated field carries a source link and a verification tier; where the public record has no answer the field reads `UNKNOWN` and names what is missing; where sources disagree it reads `CONTESTED` and shows both without adjudicating.

Its two most useful outputs are not the map:

- [**GAPS.md**](prototype/roosevelt-island/GAPS.md) — every field the public record could not answer, why, and where the data probably lives. Construction and maintenance costs are unpublished for most researched objects. The derived officeholder record is not computable, because no project register is published.
- [**SCHEMA_NOTES.md**](prototype/roosevelt-island/SCHEMA_NOTES.md) — nine places `DATA_MODEL.md` v0.1.1 broke on contact with 29 real objects. Four were predicted; five were not.

---

## Status

**Pre-alpha. Specification stage.** The core of this repository is documentation. The pilot is a manually-compiled prototype, not production code — geometry is approximate, ingestion is by hand, and the schema is draft. Do not build against it yet.
